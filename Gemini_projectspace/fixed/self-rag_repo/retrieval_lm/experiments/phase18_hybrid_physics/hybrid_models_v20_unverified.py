...
class KuramotoMemory:
    def __init__(self, dim, K=0.1, threshold=0.15):
        self.dim = dim
        self.K = K
        self.threshold = threshold
        self.active_memories = [] # list of vectors
    def add_memories(self, vectors):
        for v in vectors:
            norm = np.linalg.norm(v)
            if norm > 0: self.active_memories.append(v / norm)
    def update(self, v_in, alpha):
        v = v_in / np.linalg.norm(v_in) if np.linalg.norm(v_in)>0 else v_in
        if alpha > 0.1: # Only track significant clusters
