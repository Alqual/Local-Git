...
1.  **Trigger Limits**:
    -   **Logs**: >1,000 lines must be moved to `/l2_episodic_dock/logs/` or external DB (AWS).
    -   **Code Snapshots**: Large intermediate versions should be stored in `/experiments/` and referenced by URI.
2.  **Pointer Protocol**:
